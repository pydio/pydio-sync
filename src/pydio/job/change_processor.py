...
class ChangeProcessor:
                ...
                if item['node']['node_path']:
                    if location == 'remote':
                        ...
                        if item['type'] == 'create':
                            ...
                            self.change_store.buffer_real_operation(location, item['type'], item['node']['node_path'], item['node']['node_path'])
                    else:
    ...
    def process_UPLOAD(self, path, callback_dict=None):
        ...
        self.remote_sdk.upload(self.job_config.directory+path, self.local_sdk.stat(path), path, callback_dict, max_upload_size=max_upload_size)
        self.update_node_status(path, 'IDLE')
        message = path + ' ===============> ' + path
